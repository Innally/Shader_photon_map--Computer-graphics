...
namespace RayCast::Intersection
{
    ...
    // 这些是否相交的算法都是固定的？或许也可以改进
    HitRecord xSphere(const Ray& ray, const Sphere& s, float tMin, float tMax) {  // 看光线ray和s是否有相交，传入特定ray和特定s
        const auto& position = s.position;
        const auto& r = s.radius;
        Vec3 oc = ray.origin - position; // oc 是光线源头的坐标和圆的圆心的坐标差
        float a = glm::dot(ray.direction, ray.direction); // ray的方向和自己的点乘
        float b = glm::dot(oc, ray.direction); // ray的方向和oc的点乘
        float c = glm::dot(oc, oc) - r*r; // oc和oc的的点乘减去半径平方
        float discriminant = b*b - a*c;
        float sqrtDiscriminant = sqrt(discriminant);
        if (discriminant > 0) {
            float temp = (-b - sqrtDiscriminant) / a; //算出沿着法向距离到第一个交点的距离
            if (temp < tMax && temp > tMin) { // 如果落在这个范围（min，max）之内
                auto hitPoint = ray.at(temp); //算出hitpoint的位置
                auto normal = (hitPoint - position)/r; // 标准化
                return getHitRecord(temp, hitPoint, normal, s.material); // 传入temp，hitpoint，标准化后的值，以及sphere的材质信息
            }
